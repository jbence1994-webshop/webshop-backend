...
public class CouponTests {
    ...
    private static Stream<Arguments> couponParams() {
        return Stream.of(
                Arguments.of(Named.of("Coupon is not expired", percentOffNotExpiredCoupon()), false),
                Arguments.of(Named.of("Coupon is expired", fixedAmountExpiredCoupon()), true)
        );
    }

    @ParameterizedTest(name = "{index} => {0}")
    @MethodSource("couponParams")
    public void isExpiredTests(Coupon coupon, boolean expectedResult) {
