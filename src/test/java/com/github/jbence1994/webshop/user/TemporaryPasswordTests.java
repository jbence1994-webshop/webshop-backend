...
public class TemporaryPasswordTests {
    ...
    private static Stream<Arguments> temporaryPasswordParams() {
        return Stream.of(
                Arguments.of(Named.of("Temporary password is not expired", notExpiredTemporaryPassword()), false),
                Arguments.of(Named.of("Temporary password is expired", expiredTemporaryPassword()), true)
        );
    }

    @ParameterizedTest(name = "{index} => {0}")
    @MethodSource("temporaryPasswordParams")
    public void isExpiredTests(TemporaryPassword temporaryPassword, boolean expectedResult) {
