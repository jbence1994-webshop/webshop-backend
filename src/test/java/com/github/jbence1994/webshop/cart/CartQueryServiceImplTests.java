...
public class CartQueryServiceImplTests {
    ...
    public void getCartTest_HappyPath() {
        when(cartRepository.findById(any())).thenReturn(Optional.of(cart1()));

        var result = assertDoesNotThrow(() -> cartQueryService.getCart(CART_ID));

        assertThat(result, not(nullValue()));
        assertThat(result, allOf(
                hasProperty("id", equalTo(cart1().getId())),
                hasProperty("createdAt", equalTo(cart1().getCreatedAt()))
        ));
        assertThat(result.getItems(), not(empty()));
    }

    @Test
    public void getCartTest_UnhappyPath() {
